...
HMODULE __stdcall zLoadLibraryA(LPCSTR arg)
{
	std::string _buf = arg;

	std::wstring _wbuf(_buf.begin(), _buf.end());
	HMODULE res = LoadLibraryW(_wbuf.c_str());

	if (_buf.find("d3d9.dll", 0) > 0)
	{
		//return LoadLibraryW(L"d3d9.dll");
...
bool WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		LoadLibraryA("re_mod.dll");
		DWORD lBuf;
		lHook = (DWORD)GetProcAddress(LoadLibraryA("kernelbase.dll"), "LoadLibraryA");
		lOffset = (DWORD)&zLoadLibraryA - lHook - 5;
		//WriteProcessMemory((HANDLE)-1, (LPVOID)lHook, "\xE9", 1, &lBuf);
		//WriteProcessMemory((HANDLE)-1, (LPVOID)(lHook + 1), &lOffset, 4, &lBuf);
